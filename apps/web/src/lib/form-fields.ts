...
import { formatMoney, localise, localiseAll, optionLabels, type Locale } from '@zemmz/shared';
...
export async function loadFormFields(eventId: string, currency: string, opts: { forOrganiser?: boolean; locale?: Locale } = {}) {
  ...
  const defs: FieldDef[] = fields.map((f) => ({ key: f.key, label: localise(f, locale).label, kind: f.kind, required: opts.forOrganiser ? f.locked && f.required : f.required, enabled: f.enabled, options: f.options, optionLabels: locale === 'ar' ? optionLabels(f, locale) : undefined }));
  ...
  return {
    fields: defs,
    tickets: localiseAll(tickets, locale).map((t) => ({ id: t.id, label: `${t.name} · ${formatMoney(t.priceMinor, currency, { freeLabel: true })}${t.onSale ? '' : ' (not on sale)'}` })),
    ticketRows: tickets,
  };
}
