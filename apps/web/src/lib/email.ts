...
import { applyMergeTags, escapeHtml, eventType, formatDateRange, sanitizeRichText, stripHtml, textOn, type MergeValues } from '@zemmz/shared';
import { ticketToken } from './tokens';

export const appUrl = () => (process.env.APP_URL ?? 'http://localhost:3000').replace(/\/$/, '');

export function mergeValuesFor(event: Event, reg: Pick<Registration, 'firstName' | 'lastName' | 'title' | 'publicId'>, ticket?: Pick<TicketType, 'name'> | null): MergeValues {
  return {
    ...
    event_dates: formatDateRange(event.startsOn, event.endsOn, 'UTC'),
 ...
 */
export function emailLayout(opts: { event: Event; kicker: string; bodyHtml: string; publicId?: number; ticketUrl?: string }) {
  const { event } = opts;
  ...
  const idBlock = opts.publicId
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0 8px;border:1px solid #E6E6F0;border-radius:12px">
        <tr><td style="padding:16px 20px;font:500 12px/1.4 Arial,sans-serif;color:#6B6A85">${escapeHtml(TY.idName)}</td></tr>
        <tr><td style="padding:0 20px 16px;font:700 32px/1 Arial,sans-serif;letter-spacing:.06em;color:#0D0B2E">${opts.publicId}</td></tr>
        ${opts.ticketUrl ? `<tr><td style="padding:0 20px 18px"><a href="${escapeHtml(opts.ticketUrl)}" style="display:inline-block;background:${escapeHtml(event.accentColour)};color:${ink};font:600 14px Arial,sans-serif;text-decoration:none;padding:10px 16px;border-radius:8px">Show my ${escapeHtml(TY.badge)}</a></td></tr>` : ''}
      </table>`
    : '';
  return `<!doctype html><html lang="en"><body style="margin:0;background:#F4F5FA">
...
export function renderConfirmation(event: Event, template: Pick<MessageTemplate, 'subject' | 'bodyHtml' | 'kicker'>, reg: Registration, ticket: TicketType | null) {
  ...
  const html = emailLayout({ event, kicker: template.kicker || 'CONFIRMATION', bodyHtml: body, publicId: reg.publicId, ticketUrl });
  const text = `${stripHtml(body)}\n\n${eventType(event.type).idName}: ${reg.publicId}\n${ticketUrl}\n`;
...
export function renderBroadcast(event: Event, subjectT: string, bodyT: string, reg: Registration) {
  ...
  return {
    subject,
    html: emailLayout({ event, kicker: 'UPDATE', bodyHtml: body }),
    text: stripHtml(body),
  };
}
