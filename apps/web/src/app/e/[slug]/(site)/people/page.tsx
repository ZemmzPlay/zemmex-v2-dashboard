...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  return { title: (await siteTextFor(await getPublicEvent((await params).slug))).t.v.people };
}
...
export default async function PeoplePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (other.length) groups.push([t.term('Also appearing'), other]);

  return (
    <div className="wrap pb-20">
      <div className="page-h">
        <h1>{t.v.people}</h1>
        <p className="sec-p">{t.peopleSub}</p>
      </div>
      {people.length === 0 && <p className="text-[var(--muted)]">{t.peopleSoon}</p>}
      {groups.map(([cat, list]) => (
        <section key={cat} className="mb-12" aria-labelledby={`c-${cat}`}>
          <h2 id={`c-${cat}`} className="mb-5 text-[20px] font-bold" style={{ fontFamily: 'var(--serif)', color: 'var(--accent-link, var(--accent))' }}>{t.catHeading(cat)}</h2>
            ...
            {list.map((p) => (
              <details className="person" key={p.id}>
                <summary>
                  ...
                  <b>{p.name}</b>
                  <small>{p.setTime || (highlight.get(p.highlight) ? t.term(highlight.get(p.highlight)!) : t.catHeading(p.category))}</small>
                </summary>
                {p.bio && <p>{p.bio}</p>}
              </details>
            ))}
